...
def tokenize_exp_as_symbols_dict_bert(statement_dict):
    words_dict = dict()
    expressions_dict = dict()
    for title, content in tqdm(statement_dict.items()):
        words, expressions = tokenize_exp_as_symbols_bert(content)
        words_dict[title] = words
        expressions_dict[title] = expressions

    return words_dict, expressions_dict


def tokenize_separated(statement_dict):
    statements = dict()
    expressions = dict()
    b_tokenizer = BertTokenizer.from_pretrained("bert-base-cased")
    for title, content in tqdm(statement_dict.items()):
        statement = split_expressions(content)
        for element in statement:
            tokenized_statement = list()
            tokenized_expression = list()
            if "{{begin-" in element or element[0] == "$":
                tokenized_exp = tokenize_equation(element)
                tokenized_statement.extend(len(tokenized_exp) * ["[MASK]"])
                tokenized_expression.extend(tokenized_exp)
            else:
                tokenized_text = b_tokenizer.tokenize(element)
                tokenized_statement.extend(tokenized_text)
                tokenized_expression.extend(len(tokenized_text) * ["[MASK]"])

        statements[title] = tokenized_statement
        expressions[title] = tokenized_expression
...
def tokenize_exp_as_symbols_dict(statement_dict):
    ...
    masking = dict()
    for title, content in tqdm(statement_dict.items()):
        tokenization_s, masking_s = tokenize_exp_as_symbols(content)
        statements[title] = tokenization_s
        masking[title] = masking_s
    return statements, masking
